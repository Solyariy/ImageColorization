...
def train_one_epoch(model, loader, processor, optimizer, criterion, epoch):
    running_loss = 0
    loop = tqdm(loader, desc=f"Epoch {epoch}/{TrainingConfig.EPOCHS}")
...
def save_checkpoint(model, epoch):
    if not os.path.exists(main_config.TRAINED_MODELS):
        os.mkdir(main_config.TRAINED_MODELS)
...
def train_baseline():
    ...
    for epoch in range(1, TrainingConfig.EPOCHS + 1):
        ...
        print(f"\nEpoch [{epoch}/{TrainingConfig.EPOCHS}] Completed. Avg Loss: {avg_loss:.4f}")

        if epoch % 5 == 0 or epoch == TrainingConfig.EPOCHS:
